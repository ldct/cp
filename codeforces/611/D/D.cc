...
int n;
string S;
map<pair<int, BigInt>, long long> memo;
long long P = (10e9) + 7;

long long numWays(int a, BigInt minimumStarting) {
  if (a == n) return 1;
  if (S[a] == 'c') return 0;

  pair<int, BigInt> key = make_pair(a, minimumStarting);
  map<pair<int, BigInt>, long long>::iterator it = memo.find(key);
  ...
  for (int i=a+1; i<n+1; i++) {
    BigInt firstPart = Integer(S.substr(a, i-a));
...
int main() {
  ...
  cout << numWays(0, Integer(1)) % P << endl;
