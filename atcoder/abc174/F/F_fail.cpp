...
class SegTreeFail {
public:
  unique_ptr<SegTreeFail> lTree;
  unique_ptr<SegTreeFail> rTree;
  int leftmost;
  int rightmost;
  set<int> colours;

  SegTreeFail(vector<int>&a, int _leftmost, int _rightmost) {
    leftmost = _leftmost;
    ...
    if (leftmost == rightmost) {
      ...
      lTree = unique_ptr<SegTreeFail>(new SegTreeFail(a, leftmost, mid));
      rTree = unique_ptr<SegTreeFail>(new SegTreeFail(a, mid+1, rightmost));
...
int main() {
    ...
    auto st = SegTreeFail(colours, 0, colours.size()-1);
    for (int i=0; i<Q; i++) {
        int l, r;
        cin >> l >> r;
        l--; r--;
